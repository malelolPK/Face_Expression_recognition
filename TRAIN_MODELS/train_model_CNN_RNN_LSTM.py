...
from model_CNN_RNN import HybridCNNandRNN
from model_CNN_LSTM import HybridCNNandLSTM
from model_VISION_TRANSFORMER import Patch_embedding, EncoderBlock, Vit
...
def main():
    ...
    NUM_LAYERS_RNN = 2
    LEARNING_RATE = 2e-05
    GAMMA = 0.1 # czyli lr zostanie przez to pomnożone
    ...
    # Transformata użyta dla obrazów
    transform = v2.Compose([
        v2.Resize(size=(48, 48)),
        v2.ToImage(),
        v2.AutoAugment(),
        v2.ToDtype(torch.float32, scale=True)
    ]) # wyglądaja na znormalizowane
    ...
    # model_01 = HybridCNNandRNN(input_shape_cnn=3,
    #                            hidden_units_cnn=HIDDEN_UNITS,
    #                            hidden_units_rnn=HIDDEN_UNITS_RNN,
    #                            output_shape=len(class_names),
    #                            num_layers_rnn=NUM_LAYERS_RNN).to(device)
    model_01 = HybridCNNandLSTM(input_shape_cnn=3,
                               hidden_units_cnn=HIDDEN_UNITS,
                               hidden_units_rnn=HIDDEN_UNITS_RNN,
                               output_shape=len(class_names),
                               num_layers_rnn=NUM_LAYERS_RNN).to(device)
    ...
    lr_schedule = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[150, 200], gamma=GAMMA)
    # Trenowanie modelu

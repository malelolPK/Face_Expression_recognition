...
from model_VISION_TRANSFORMER import Vit
...
def main():
    ...
    MILESTONES = [150,200] # set a scheduler to decay the learning rate by 0.1 on the 100th 150th epochs
    L2_WEIGHT_DECAY = 3e-5
    EPOCHS = 5


    EMB_SIZE = 256 # EMB_SIZE powinno być podzielne przez NUM_HEADS
    NUM_HIDDEN_LAYERS = 4 * EMB_SIZE
    PATCH_SIZE = 4
    C_CHANNELS = 3
    NUM_HEADS = 8
    NUM_LAYERS_ATTEN = 8
    ...
    model_01 = Vit(emb_size=EMB_SIZE,
                   num_heads=NUM_HEADS,
                   num_leyers = NUM_LAYERS_ATTEN,
                   num_hidden_layers=NUM_HIDDEN_LAYERS,
                   C_channels = C_CHANNELS,
                   patch_size=PATCH_SIZE,
                   num_class=len(class_names),
                   dropout=DROPUT).to(device)
    ...
    lr_schedule = optim.lr_scheduler.MultiStepLR(optimizer, milestones=MILESTONES, gamma=GAMMA_LR_SCHEDULER)
    # Trenowanie modelu
